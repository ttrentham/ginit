...
module.exports = {
  ...
  getRemoteReposforOrg: async (organization) => {
    ...
    const status = new Spinner('Getting Org repositories...');
    status.start();

    try {

      const options = github.repos.listForOrg.endpoint.merge(data);
      github.paginate(options)
        .then(repos => {
          // repos is an array of all issue objects
          let result = repos.map(a => a.name);
          return result;
        })      

      //const response = await github.repos.listForOrg(data);
